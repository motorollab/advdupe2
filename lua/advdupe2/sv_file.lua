...
local function SaveFile( ply, cmd, args )
    ...
    if (CurTime() - (ply.AdvDupe2.FileMod or 0) < 0) then
...
function AdvDupe2.SendToClient( ply, data, autosave )
    ...
    if #data > AdvDupe2.MaxDupeSize then
        return AdvDupe2.Notify( ply, "Copied duplicator filesize is too big!", NOTIFY_ERROR )
    end
    ...
    timer.Create( "bulk_adv2_timeout", 5, 1, function()
        ...
        AdvDupe2.Notify( ply, message, NOTIFY_ERROR )
    end )
...
function AdvDupe2.LoadDupe( ply, success, dupe, info, moreinfo )
    ...
    if (tonumber( GetConVarString( "AdvDupe2_MaxConstraints" ) ) ~= 0 and #dupe["Constraints"] >
        tonumber( GetConVarString( "AdvDupe2_MaxConstraints" ) )) then
...
local function AdvDupe2_ReceiveFile( ply, data )
    ...
    if not data then
        return AdvDupe2.Notify( ply, "Duplicator Upload Failed!", NOTIFY_ERROR, 5 )
    end

    ply.AdvDupe2 = ply.AdvDupe2 or {}
    ply.AdvDupe2.Name = string.match( data.name, "([%w_ ]+)" ) or "Advanced Duplication"

    AdvDupe2.DecodeAsync( data.read, function( success, dupe, info, moreinfo )
