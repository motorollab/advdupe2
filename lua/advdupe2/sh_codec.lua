...
local function serialize( tbl )
    ...
    if not buff then
        error( "Failed to open file data/ad2temp.txt for writing!" )
    end
    write( tbl )
    buff:Close()
    ...
    if not buff then
        error( "Failed to open file data/ad2temp.txt for reading!" )
    end
...
versionsAsync[4] = function( encodedDupe, cb )
    collectgarbage( "stop" )
    local info, dupestring = getInfo( encodedDupe:sub( 7 ) )
    flzma.DecompressAsync( dupestring, function( d )
        local s, t = pcall( deserialize, d, read4 )
        cb( s, t, info )
        collectgarbage( "restart" )
    end )
end

versionsAsync[5] = function( encodedDupe, cb )
    collectgarbage( "stop" )
    local info, dupestring = getInfo( encodedDupe:sub( 7 ) )
    flzma.DecompressAsync( dupestring, function( d )
        local s, t = pcall( deserialize, d, read5 )
        cb( s, t, info )
        collectgarbage( "restart" )
    end )
end
...
function AdvDupe2.CheckValidDupe( dupe, info )
    if not dupe.HeadEnt then
        return false, "Missing HeadEnt table"
    end
    if not dupe.Entities then
        return false, "Missing Entities table"
    end
    if not dupe.Constraints then
        return false, "Missing Constraints table"
    end
    if not dupe.HeadEnt.Z then
        return false, "Missing HeadEnt.Z"
    end
    if not dupe.HeadEnt.Pos then
        return false, "Missing HeadEnt.Pos"
    end
    if not dupe.HeadEnt.Index then
        return false, "Missing HeadEnt.Index"
    end
    if not dupe.Entities[dupe.HeadEnt.Index] then
        return false, "Missing HeadEnt index [" .. dupe.HeadEnt.Index .. "] from Entities table"
    end
    for key, data in pairs( dupe.Entities ) do
        if not data.PhysicsObjects then
            return false, "Missing PhysicsObject table from Entity [" .. key .. "][" .. data.Class .. "][" .. data.Model .. "]"
        end
        if not data.PhysicsObjects[0] then
            return false, "Missing PhysicsObject[0] table from Entity [" .. key .. "][" .. data.Class .. "][" .. data.Model .. "]"
        end
        if info.ad1 then -- Advanced Duplicator 1
            if not data.PhysicsObjects[0].LocalPos then
                return false, "Missing PhysicsObject[0].LocalPos from Entity [" .. key .. "][" .. data.Class .. "][" .. data.Model .. "]"
            end
            if not data.PhysicsObjects[0].LocalAngle then
                return false, "Missing PhysicsObject[0].LocalAngle from Entity [" .. key .. "][" .. data.Class .. "][" .. data.Model .. "]"
            end
        else -- Advanced Duplicator 2
            if not data.PhysicsObjects[0].Pos then
                return false, "Missing PhysicsObject[0].Pos from Entity [" .. key .. "][" .. data.Class .. "][" .. data.Model .. "]"
            end
            if not data.PhysicsObjects[0].Angle then
                return false, "Missing PhysicsObject[0].Angle from Entity [" .. key .. "][" .. data.Class .. "][" .. data.Model .. "]"
            end
        end
    end
    return true, dupe
    ...
    if sig ~= "AD2F" then
        if sig == "[Inf" then -- legacy support, ENGAGE (AD1 dupe detected)
            ...
            if success then
                ...
                if not index then
                    return false, "Missing head index"
                end
                local pos
                ...
                tbl.HeadEnt = { Index = index, Pos = pos, Z = z }
            else
                ErrorNoHalt( tbl )
            end
...
---@param encodedDupe string
---@param callback function
function AdvDupe2.DecodeAsync( encodedDupe, callback )
    ---@diagnostic disable-next-line
    if not flzma then
        return callback( AdvDupe2.Decode( encodedDupe ) )
    end
    ...
    elseif rev < 1 then
        ...
        if versionsAsync[rev] then
            versionsAsync[rev]( encodedDupe, callback )
        else
            local success, tbl, info = pcall( versions[rev], encodedDupe )
            if success then
                success, tbl = AdvDupe2.CheckValidDupe( tbl, info )
...
if CLIENT then
    ...
    concommand.Add( "advdupe2_to_json", function( _, _, arg )
        if not arg[1] then
            print( "Need AdvDupe2 file name argument!" )
            return
        end
        ...
        if not readFile then
            print( "File could not be read or found! (" .. readFileName .. ")" )
            return
        end
        ...
        if not writeFile then
            print( "File could not be written! (" .. writeFileName .. ")" )
            return
        end
